"""
Agent 3: Cross-Lingual Transfer Agent
=======================================
The "language bridge" specialist.

Responsibilities:
- When an error is flagged, check if the student's known languages
  (Mandarin, English) can help explain WHY the error happened
- Find positive transfers: "Mandarin works like Japanese here, use that!"
- Find negative transfers: "English misleads you here, watch out!"
- If neither language helps, say so honestly

This agent uses a pre-built transfer map (JSON file) as its knowledge base,
supplemented by the LLM's general language knowledge.
"""
